...
- Flutter SDK is installed at `D:\flutter` and is **not on PATH** — call `D:\flutter\bin\flutter.bat` if `flutter` isn't found.
- `android/local.properties` (machine-specific `sdk.dir` + `flutter.sdk`) is generated by Flutter and gitignored — fix it when moving machines.
- Building the Android APK requires Gradle, which works from Android Studio and a normal terminal. Note for AI agents: some sandboxed execution environments block Gradle's JVM loopback connection (`java.io.IOException: Unable to establish loopback connection`) — if you hit that, the APK build must be run by the user, not the agent. `flutter analyze` and `flutter build windows` are unaffected.
